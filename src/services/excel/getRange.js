...
async function getRange1(inf) {
    ...
    const requisicao = {
        url: `https://graph.microsoft.com/v1.0/me/drive/items/${fileId}/workbook/worksheets/${sheetTabId}/range(address='A1:Z1')`,
        method: 'GET',
        headers: {
...
async function run() {
    const resultado = await getRange1()
    if (resultado.error.code == 'undefined') {
        ...
        const resultado = await getRange1(session);
